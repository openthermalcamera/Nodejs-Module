...
function getTemp(data)
{
	...
	ktaScale = Math.pow(2, mlx.ktaScale);
	kvScale = Math.pow(2, mlx.kvScale);
	alphaScale = Math.pow(2, mlx.alphaScale);
	...
	for (var pixelNumber = 0; pixelNumber < 768; pixelNumber++)
	{
		...
		if (pattern === frameData[833])
		{
			...
			kta = mlx.kta[pixelNumber] / ktaScale;
			kv = mlx.kv[pixelNumber] / kvScale;
			irData = irData - mlx.offset[pixelNumber] * (1 + kta * (ta - 25)) * (1 + kv * (vdd - 3.3));
			...
			alphaCompensated = SCALEALPHA * alphaScale / mlx.alpha[pixelNumber];
			alphaCompensated = alphaCompensated * (1 + mlx.KsTa * (ta - 25));
...
function ExtractAlpha(eeData)
{
	...
	for (var i = 0; i < 24; i++)
	{
		for (var j = 0; j < 32; j++)
		{
			...
			alphaTemp[p] = alphaTemp[p] / Math.pow(2, alphaScale);
			alphaTemp[p] = alphaTemp[p] - mlx.tgc * (mlx.cpAlpha[0] + mlx.cpAlpha[1]) / 2;
	...
	for (var i = 0; i < 768; i++)
	{
		...
		mlx.alpha[i] = parseInt(temp + 0.5);
		if(mlx.alpha[i] > 32767) mlx.alpha[i] -= 65536
		//mlx.alpha is signed, if above 32k, subtract 65k
	}
	mlx.alphaScale = alphaScale;
}
...
function ExtractKtaPixel(eeData)
{
	...
	for (var i = 0; i < 24; i++)
	{
		for (var j = 0; j < 32; j++)
		{
			...
			ktaTemp[p] = ktaTemp[p] / Math.pow(2, ktaScale1);
...
function ExtractKvPixel(eeData)
{
	...
	for (var i = 0; i < 24; i++)
	{
		for (var j = 0; j < 32; j++)
		{
			...
			kvTemp[p] = KvT[split];
			kvTemp[p] = kvTemp[p] / Math.pow(2, kvScale);
